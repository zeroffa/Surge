...
腳本相容: Surge、QuantumultX、Loon、Shadowrocket、Node.js
...
2. 不建議在凌晨執行,因需要獲取動畫瘋題目答案; 默認組態將在每天的早上8:00執行.
...
})().catch((e) => $.notifyMsg.push(e.message || e)) //捕獲登錄函數等拋出的異常, 並把原因新增到全域變量(通知)
...
async function BahamutLogin(retry = 3, interval = 1000) { //登錄函數，拿到Set-Cookie
	...
	for (let i = 0; i < retry; i++) { //循環登錄(默認三次)
		...
		const reqUrl = {
			url: 'https://api.gamer.com.tw/mobile_app/user/v3/do_login.php', //登錄介面
			headers: { //請求頭
		...
		const res = await $.http.post(reqUrl) //使用post請求查詢 (相容函數實際上返回Promise實例對象,以便後續調用時可以實現順序執行異步函數)
			.then(async (resp) => { //請求成功的處理
				const body = JSON.parse(resp.body); //解析響應體json為對象
				if (body.userid) { //如果成功返回用戶資訊
...
function BahamutSign() { //查詢巴哈姆特簽到Token
	return $.http.get({ //使用get方法 (Promise實例對象) 查詢簽到Token
			url: 'https://www.gamer.com.tw/ajax/get_csrf_token.php', // 查詢Token介面
			headers: {} //請求頭, 客戶端將自動設置Cookie欄位
			...
			if (resp.body) { //如果簽到Token獲取成功
				...
				$.notifyMsg.push(`首頁簽到: 成功, 已連續簽到 ${sign} 天`); //新增到全域變量備用 (通知)
		...
		.catch(err => {
			$.notifyMsg.push(`首頁簽到: ${err.message||err}`); //新增到全域變量備用 (通知)
...
function StartSignBahamut(token) { //巴哈姆特簽到
	...
	return $.http.post({ //使用post方法 (Promise實例對象) 進行簽到
			url: 'https://www.gamer.com.tw/ajax/signin.php', //巴哈姆特簽到介面
			headers: {}, //請求頭, 客戶端將自動設置Cookie欄位
		...
		.then(res => { // 網絡請求成功的處理
			...
			if (body.data) { // 如果簽到成功 (判斷預期響應格式)
				$.log('', '✅巴哈姆特簽到成功', `✅已連續簽到 ${body.data.days} 天`); //列印日誌
...
function StartAdsBonus(token, type) {
	...
	return $.http.post({ //使用post方法 (Promise實例對象) 進行簽到
			url: 'https://api.gamer.com.tw/mobile_app/bahamut/v1/sign_in_ad_' + type + '.php?bahamutCsrfToken=' + token, //雙倍巴幣廣告獎勵介面
			headers: {
		...
		.then(async (res) => { //網絡請求成功的處理, 實例函數帶有async關鍵字, 表示裡面有異步操作
			const body = JSON.parse(res.body); //解析響應體json為對象
			if (body.data && body.data.finished == 0 && type == 'start') { //如果成功啟動廣告獎勵
				$.log('', '🔶正在執行廣告簽到 (30s)'); //列印日誌
				await $.wait(30000); //等待30秒
				await StartAdsBonus(token, 'finished'); //領取獎勵函數
			} else if (body.data && body.data.finished == 1) { //如果廣告獎勵領取成功
				$.log('', '✅領取廣告獎勵成功'); //列印日誌
				$.notifyMsg.push('廣告簽到: 成功, 已領取雙倍簽到獎勵'); //新增到全域變量備用 (通知)
			} else {
				const failMsg = body.error ? body.error.message : null; //判斷簽到失敗原因
				throw new Error(failMsg || body.message || '未知'); //帶上原因拋出異常
			}
		...
		.catch(err => {
			$.notifyMsg.push(`廣告簽到: ${err.message||err}`); //新增到全域變量備用 (通知)
...
function BahamutGuildSign() { //巴哈姆特查詢公會列表
	...
	return $.http.get({ //使用get請求查詢公會列表 (Promise實例對象)
			url: 'https://api.gamer.com.tw/ajax/common/topBar.php?type=forum', // 查詢公會列表介面
			headers: {} //請求頭, 客戶端將自動設置Cookie欄位
		})
		.then(async (resp) => { //網絡請求成功的處理, 實例函數帶有async關鍵字, 表示裡面有異步操作
		...
		.catch(err => { //捕獲異常, 列印日誌
			$.notifyMsg.push(`公會簽到: ${err.message || err}`); //新增到全域變量備用 (通知)
			$.log('', `❌巴哈姆特公會簽到失敗`, `❌${err.message || err}`); //列印日誌
		});
}
...
function StartSignGuild(v) { //巴哈姆特公會簽到
	...
	return $.http.post({ //使用post方法簽到公會 (Promise實例對象)
			url: 'https://guild.gamer.com.tw/ajax/guildSign.php', //公會簽到介面
			headers: {}, //請求頭, 客戶端將自動設置Cookie欄位
			body: `sn=${v.sn}` //把查詢到的公會ID放進請求體
		})
		.then((res) => { //網絡請求成功後的處理
			const body = JSON.parse(res.body); //解析響應體json為對象
			$.log('', `🔷<${v.name}>`, `${body.ok?`✅`:`❌`}${body.msg}`); //列印日誌, 包含簽到結果
			if (body.ok) { //如果簽到成功
				return 1; //返回1表示成功
			} else {
				return 0; //返回0表示失敗
...
function BahamutAnswer() { //動畫瘋答題
	...
	return $.http.get({ //使用get方獲取題目 (Promise實例對象)
			url: 'https://ani.gamer.com.tw/ajax/animeGetQuestion.php?t=' + Date.now(), //獲取題目介面
			headers: {} //請求頭, 客戶端將自動設置Cookie欄位
		})
		.then(async (res) => { //網絡請求成功的處理, 實例函數帶有async關鍵字, 表示裡面有異步操作
			...
			if (r.token) { //如果有題目
				$.log('', `✅獲取動畫瘋題目成功`, ``, `🔶<${r.game}> ${r.question}`,
					`1️⃣${r.a1}`, `2️⃣${r.a2}`, `3️⃣${r.a3}`, `4️⃣${r.a4}`); //列印日誌
				...
				$.notifyMsg.push(`動畫答題: ${sendAnswer}`); //答題後的結果新增到全域變量備用 (通知)
			} else { //未獲取到題目
				throw new Error(r.msg || `獲取題目失敗`); //帶上原因拋出異常
			}
		})
		.catch(e => { //捕獲異常, 列印日誌
			$.notifyMsg.push(`動畫答題: ${e.message||e||`失敗`}`); //新增到全域變量備用 (通知)
			$.log('', `❌動畫瘋答題失敗`, `❌${e.message||e}`); //列印日誌
		});
}
...
function GetAanswerArticles() { // 從blackxblue的小屋查詢含答案的文章ID
	...
	return $.http.get({ //使用get方法獲取文章ID (Promise實例對象)
			url: 'https://api.gamer.com.tw/mobile_app/bahamut/v1/home.php?owner=blackXblue&page=1', //獲取文章ID介面
			headers: {}
		})
		.then((res) => { //網絡請求成功後的處理
			const body = JSON.parse(res.body); //解析響應體json為對象
...
function StartSearchAnswers(id) { //獲取文章內答案
	...
	return $.http.get({ //使用get方法獲取答案 (Promise實例對象)
			url: 'https://api.gamer.com.tw/mobile_app/bahamut/v1/home_creation_detail.php?sn=' + id, //獲取答案介面
			headers: {}
		})
		.then((res) => { //網絡請求成功後的處理
			const body = JSON.parse(res.body); //解析響應體json為對象
...
function StartBahamutAnswer(answer, token) { //動畫瘋答題
	...
	return $.http.post({ //使用post方法提交答案 (Promise實例對象)
			url: 'https://ani.gamer.com.tw/ajax/animeAnsQuestion.php', //提交答案介面
			headers: {}, //請求頭, 客戶端將自動設置Cookie欄位
...
//修改自 https://github.com/chavyleung/scripts/blob/master/Env.js 的相容函數
...
// 從 https://jsfiddle.net/russau/rbyjk774 魔改的TOTP兩部驗證演算法, 完全使用原生javascript實現
